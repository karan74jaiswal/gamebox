...
export interface FollowCameraRig {
  ...
  setTarget: (next: THREE.Object3D | null) => void
  stop?: () => void
}
...
export interface TopDownCameraRig {
  target: THREE.Object3D | null
  height: number
  stiffness: number
  stop?: () => void
...
export function topDownCamera(
  ...
  const camera = engine.camera
  const desired = new THREE.Vector3()
  const rig: TopDownCameraRig = { target, height, stiffness }
...
export interface SideCameraRig {
  target: THREE.Object3D | null
  distance: number
  stiffness: number
  stop?: () => void
...
export function sideCamera(
  ...
  const {
    ...
    stiffness = 5,
    offsetY = 1.5,
    deadzone: deadzoneValue = 1.2,
  } = options
  const camera = engine.camera
  const focus = new THREE.Vector3()
  const rig: SideCameraRig = { target, distance, stiffness }
  ...
  rig.stop = engine.onLateUpdate((dt) => {
    ...
    if (Math.abs(dx) > deadzoneValue) focus.x += dx - Math.sign(dx) * deadzoneValue
    if (Math.abs(dy) > deadzoneValue) focus.y += dy - Math.sign(dy) * deadzoneValue
...
export interface ThirdPersonOptions {
  ...
  jump?: number
  turnRate?: number
  relativeTo?: THREE.Camera | THREE.Object3D
}
...
export function thirdPerson(
  ...
  const controller: ThirdPersonController = {
    object,
    body,
    speed,
    velocity,
...
export function platformer(
  ...
  const controller: PlatformerController = {
    object,
    body,
    velocity,
    speed,
 ...
 */
export function pointerOnGround(
  engine: Engine,
  input: InputManager,
  height: number = 0
): () => THREE.Vector3 | null {
